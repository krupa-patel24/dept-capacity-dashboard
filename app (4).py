...
st.caption("🔵 Cumulative Spent  🟠 Threshold (Active Jobs × 8 hrs)")

# ── Load Data ──────────────────────────────────────────────────────────────────
@st.cache_data
def load_data(path="final_dedicated_jobs.csv"):
...
uploaded_file = st.sidebar.file_uploader("📂 Upload CSV (or place it next to app.py)", type="csv")
...
if uploaded_file:
    ...
    try:
        ...
        st.warning("⬆️ Please upload your `final_dedicated_jobs.csv` using the sidebar.")
        st.stop()
...
col3.metric("Cumulative Threshold", f"{last['thr_cum']:.0f} hrs")
col4.metric("Difference",           f"{delta_val:+.1f} hrs",
...
fig.update_layout(
    title=dict(
        text=(
            ...
            f'🔵 Cumulative Spent  🟠 Threshold (active jobs x 8 hrs)</sub>'
        ),
...
dept_col = None
for candidate in ['department_group', 'department', 'dept', 'Department', 'Dept', 'DEPARTMENT', 'Department_Group', 'DEPARTMENT_GROUP']:
    if candidate in proj.columns:
        dept_col = candidate
        break

if dept_col is None:
    st.info("ℹ️ No `department` column found in your data. Add a `department` column to enable this breakdown.")
else:
    dept_agg = (
    ...
    for i, (_, row) in enumerate(dept_agg.iterrows()):
        ...
        dept_cols[i].markdown(
            ...
            ">
                ...
                <div style="font-size:26px; font-weight:700; color:{color}; margin-bottom:10px;">{hrs:.2f} hrs</div>
                <div style="background:#2a2d3a; border-radius:4px; height:6px; margin-bottom:8px;">
                    <div style="background:{color}; width:{min(pct,100):.1f}%; height:6px; border-radius:4px;"></div>
                </div>
